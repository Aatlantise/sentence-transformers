...
# model
def complete_HAC(dataset, HAC_dist, k, datatype=np.int32, batch_size = 10000):
    ...
    for count,idx_pair in enumerate(idx_list):
        left_data[batch_count]=dataset[idx_pair[0]] #error here
...
def Louvain_no_isolation(dataset, edge_measure, datatype=np.int32, cluster_batch_size = 10000, iso_thres=5):
    ...
    for count,idx_pair in enumerate(idx_list):
        ...
        if batch_count == batch_size:
            print('predicting...',str(round(count/len(idx_list)*100,2))+'%')
            temp_edge_list = edge_measure(left_data,right_data) # error occurs here
            edge_list = edge_list + temp_edge_list.reshape(batch_size).tolist()
            batch_count = 0
    if batch_count !=0:
        print('predicting...')
